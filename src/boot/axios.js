...
const mockIt = (url, method, opts) => {
  ...
  const theMock = Mocks.find((mk) => mk.method === method && new RegExp(mk.url).test(url))
...
export default defineBoot(({ app }) => {
  ...
  const getRequest = (url, options, newWin = false) => {
    ...
    return (
      mockIt(`${url}${queryString}`, 'get', options) ||
      axiosInstance.get(encodeURI(url) + queryString, shouldCancel)
    )
  }
  ...
  const postRequest = (url, data, opts) => {
    ...
    return (
      mockIt(url, 'post', data) ||
      axiosInstance.post(url, opts?.__ignoreDecycle ? data : Object.decycle(data), opts)
    )
  }
  ...
  const putRequest = (url, data, opts) => {
    ...
    return (
      mockIt(url, 'put', data) ||
      axiosInstance.put(url, opts?.__ignoreDecycle ? data : Object.decycle(data), opts)
    )
  }
  ...
  const deleteRequest = (url, data, opts) => {
    ...
    return (
      mockIt(url, 'delete', { data }) ||
      axiosInstance.delete(url, { data: opts?.__ignoreDecycle ? data : Object.decycle(data) }, opts)
    )
  }
